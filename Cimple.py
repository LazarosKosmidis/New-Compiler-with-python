...
# ***************************************************************************	
	def block():
		...
		if(g5 == 5 and sect!= 1):
			...
			FinalCodeList.append("\t"+"\t"+"move $s0, $sp")
			FinalCodeCounter+=1
		if(WriteOnC == 1):	  
			if(sect==0):   
				start = "int main(){"
							...
							if(FunctionList[c][2] == " "):
								if(FunctionList[c][3] == "in"):
									...
									for bc in range (0,len(ArrayOfSyms)):
										if(FunctionList[c][1]==ArrayOfSyms[bc][0]):
								...
								if(FunctionList[c][4] == "in"):
									...
									for bc in range (0,len(ArrayOfSyms)):
										if(FunctionList[c][2]==ArrayOfSyms[bc][0]):
											FinalCodeList.append("L" + str(FinalCodeCounter) + ": " + "\t" + "addi $fp,$sp, offset")
											FinalCodeList.append("\t"+"\t"+"lw $t1,-"+str(ArrayOfSyms[bc][2])+"($sp)")
											FinalCodeList.append("\t"+"\t"+"sw $t1,-"+str(ArrayOfSyms[bc][2])+"($fp)")
											FinalCodeCounter+=1
				...
				if(gor == 0 and gor1 == 0): 
					...
					for bc in range (0,len(ArrayOfSyms)):
						if(id==ArrayOfSyms[bc][0]):
							...
							FinalCodeList.append("\t"+"\t"+"sw $t1,-"+str(ArrayOfSyms[bc][2])+"($s0)")
							FinalCodeCounter+=1
					if(WriteOnC==1):
				...
				if(WriteOnC==0):
					if(gor1==0):
						if(jpeg == "="):
							...
							counter+=1
							CList.append("\t"+"L_"+str(counter)+" "+"got to;" + "	// ( if " + jpeg + w1 + opr + ")")
							Clistlength = len(CList)
							counter+=1
						...
						if(jpeg == ">"):		
							...
							counter+=1
							CList.append("\t"+"L_"+str(counter)+" "+"got to;" + "	// ( if " + jpeg + w1 + opr1 + ")")
							Clistlength = len(CList)
							counter+=1
				
						if(jpeg == "<"):
							CList.append("\t"+"L_"+str(counter)+" if ("+fd+jpeg+opr2+");"+" goto " + "	// ( if " + jpeg + w1 + opr2 + ")")
							counter+=1
							CList.append("\t"+"L_"+str(counter)+" "+"got to;" + "	// ( if " + jpeg + w1 + opr2 + ")")
							Clistlength = len(CList)
							counter+=1
				
						if(jpeg == ">="):
							CList.append("\t"+"L_"+str(counter)+" if ("+fd+jpeg+opr3+");"+" goto " + "	 // ( if " + jpeg + w1 + opr3 + ")")
							counter+=1
							CList.append("\t"+"L_"+str(counter)+" "+"got to;" + "	// ( if " + jpeg + w1 + opr3 + ")")
							Clistlength = len(CList)
							counter+=1
				
						if(jpeg == "<="):
							CList.append("\t"+"L_"+str(counter)+" if ("+fd+jpeg+opr5+");"+" goto " + "	 // ( if " + jpeg + w1 + opr4 + ")")
							counter+=1
							CList.append("\t"+"L_"+str(counter)+" "+"got to;" + "	// (JUMP,  ,  , )")
							Clistlength = len(CList)
							counter+=1
			...
			if token.tokenString=='(':
				...
				if token.tokenString==')':
					...
					if(goe == 1):
						...
						for bc in range (0,len(ArrayOfSyms)):
							if(w1==ArrayOfSyms[bc][0]):
								FinalCodeList.append("L"+str(FinalCodeCounter)+": "+"\t"+"lw $t1,-"+str(ArrayOfSyms[bc][2])+"offset($sp)")
								FinalCodeList.append("\t"+"\t"+"li $v0,1")
								FinalCodeList.append("\t"+"\t"+"move $a0, $t1")
								FinalCodeList.append("\t"+"\t"+"syscall")
								FinalCodeCounter+=1
					if(goe != 1 and gor == 1):
						genquad("retv  ",w1,"  _ "," _ ")
	...
	def inputStat():
		...
		if token.tokenString=='input':
			...
			if token.tokenString=='(':
				...
				FinalCodeList.append("\t"+"\t"+"syscall")
				FinalCodeList.append("\t"+"\t"+"sw $v0,-offset($sp)")
				FinalCodeCounter+=1
				if token.tokenString==')':
